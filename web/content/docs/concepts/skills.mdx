...
Skills are persona-agnostic. The same `audiobooks` skill works for AbuelOS (warm Spanish narration of Spanish-language books) and for a hypothetical English persona (terse English narration of English books). Personas configure skills; skills don't know about personas.
...
The store is namespaced — your skill only sees its own keys. Storage is shared per persona (so all skills loaded by AbuelOS write to the same database) but isolated by key prefix.
