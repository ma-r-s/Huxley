...
<Callout type="warn">
  **Today's constraint snippets are Spanish-only** — the constraint registry was authored alongside AbuelOS. Personas in other languages still get the rule injected, just in Spanish text. A future change will localize the snippets per persona language; until then, English/French personas may see slightly less effective enforcement of these rules.
</Callout>
