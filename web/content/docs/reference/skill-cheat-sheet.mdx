...
| `await ctx.inject_turn_and_wait(prompt, ...)` | Like `inject_turn`, but blocks until the model finishes (falls back to plain enqueue if the coordinator is busy) |
...
- `NORMAL` — fires immediately if idle; otherwise queues until the next quiet turn-end (no pending content stream).
- `BLOCK_BEHIND_COMMS` — preempts CONTENT, yields to COMMS. Default for timers/reminders.
- `PREEMPT` — preempts everything, including phone calls. Emergency only.
