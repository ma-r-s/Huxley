...
- **No default.** If your `server/personas/` has exactly one persona, the loader picks it automatically. With two or more (the repo ships `abuelos` and `basic`), you must set this explicitly or startup fails with a clear error.
...
```bash
HUXLEY_PERSONA=basic uv run huxley     # one-off
```
