...
### Don't translate-then-call-it-done

Run the English prompt through Google Translate, paste the Spanish into the `es` block, ship. Result: a stilted, awkward Spanish persona that the model still kind of speaks but feels off. Spanish speakers who interact with it will notice immediately.

The fix: write each language's prompt *natively*. Get a fluent speaker to review. The behaviors should match across languages, but the prose shouldn't be a transliteration.

### Don't add behaviors in one language only

If your English prompt says "be brief" and your Spanish prompt also says it, fine. But if the Spanish prompt adds "and use Spanish idioms," and the English doesn't add the equivalent — the personas behave differently in different languages.

This matters because the *user* expects consistent behavior. If they're bilingual and switch the toggle mid-conversation, the agent shouldn't suddenly become a different agent. Match instructions across all language overrides.
