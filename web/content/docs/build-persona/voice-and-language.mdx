...
- `alloy` — neutral, balanced. Fine for default-feeling agents.
- `coral` — warm, slightly slow. Excellent for elderly-facing or accessibility-driven personas.
- `echo` — formal, clipped. Good for business contexts.
- `shimmer` — light, quick. Good for energetic, kid-friendly personas.
- `verse` — expressive, stylized. Polarizing — try before you commit.
- `sage` — calm, measured. Good for meditative or therapeutic apps.

Try them all in [OpenAI's playground](https://platform.openai.com/playground) before picking. Listen to each one say a sentence in your persona's language. The voice has more impact than the prompt.

A few practical notes:

- **Spanish, Portuguese, Italian** — `coral` is widely loved. `alloy` works.
- **English** — every voice works; pick by character.
- **French, German, Japanese, etc.** — quality varies; `alloy` is the safest starting point. Test before committing.
