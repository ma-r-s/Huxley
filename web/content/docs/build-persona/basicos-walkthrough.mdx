...
Here's `server/personas/basicos/persona.yaml`. Unlike a complex persona, this one is short enough to read in full:
...
No sounds directory, no media library — just the database the framework creates on first run. This is the minimum viable persona directory.
...
- **The skill list** — add skills for everything your user actually needs. Remove `news` if they don't need headlines. Add `timers` if they need reminders.
