...
Opens at `http://localhost:5174`. The PWA reads its server list from the `VITE_HUXLEY_PERSONAS` env var — a comma-separated list of `name:url` pairs (e.g. `abuelos:ws://localhost:8765,basic:ws://localhost:8766`). The in-app picker switches between them.
