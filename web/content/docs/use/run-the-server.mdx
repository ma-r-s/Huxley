...
This loads `.env` from the current directory, reads `HUXLEY_PERSONA`, and listens on `:8765`. With multiple personas in `server/personas/` (the repo ships `abuelos` and `basic`), set `HUXLEY_PERSONA` explicitly. Restart between code changes — there's no hot reload for the server (skills hold state; reloads would lose it).
...
If you do this, make sure their database paths don't collide — by default each persona has its own (`server/personas/abuelos/data/`, `server/personas/basic/data/`).
