...
**Idle sessions are free.** OpenAI bills per Response, not per connected session. We keep Realtime sessions open during media playback so the user has continuous conversational context, and the cost is zero. (Verified — see [the cost model memory](/docs/use/run-the-server) section on cost.)
