...
1. **`PersonaSummary.name` conflated wire id with display label.** `list_personas()` returned `spec.name` (the YAML's `name:` field, e.g. `"Buddy"`, `"Basic"`) as the persona's `name` — but `?persona=` looks up the **directory basename** (`buddy/`, `basic/`) for filesystem resolution. macOS's case-insensitive FS made `Buddy → buddy/` work by accident (same length); `Basic → basic/` failed (different length). Critics couldn't catch this — they reasoned about the contract; the bug was in the runtime mapping.
