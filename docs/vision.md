...
The first persona shipped on Huxley is **AbuelOS** — a Spanish-language assistant for an elderly blind user, built around a "never say no" behavioral constraint. Its spec lives in [`personas/abuelos.md`](./personas/abuelos.md) and serves as the worked example for everything in this repo.
