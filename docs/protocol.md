...
- **Query parameters at handshake** (all optional):
  - `?lang=<code>` — selects the persona's translation for this session (e.g. `es-CO`, `en`). The server resolves the persona for that language; an unsupported code falls back to the persona's default silently.
  - `?persona=<name>` — **T1.13.** Selects which persona this connection is for. The server swaps the active `Application` to that persona before sending hello, so the hello payload reflects the new persona. Omit the param to keep the currently-active persona; on first connection, the server uses `HUXLEY_PERSONA` env or falls back per [persona resolution](#persona-selection-via-query-param-t113). An unknown name is logged and the connection still establishes on the previous persona — the picker UI can detect the mismatch via `hello.current_persona` and surface a retry. Persona-switch is exclusively a reconnect operation; there is no in-band `select_persona` message. Rationale: [decision: hot persona swap via reconnect](./decisions.md#2026-05-01--hot-persona-swap-via-reconnect-not-in-band-t113).
...
| `list_sessions`  | —                                  | **T1.12.** Ask the server for the persisted conversation history. Server replies with `sessions_list`. PWA dispatches when the SessionsSheet opens.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                  |
| `get_session`    | `{ id: number }`                   | **T1.12.** Ask for the full transcript of one session. Server replies with `session_detail`. PWA dispatches when a session row is clicked. Server logs and drops malformed `id`s.                                                                                                                                                                                                                                                                                                                                                                                                                                                                    |
| `delete_session` | `{ id: number }`                   | **T1.12.** Privacy floor — remove the session row and its turns. Server replies with `session_deleted` once the rows are gone. PWA dispatches from the SessionDetailSheet "Delete" button.                                                                                                                                                                                                                                                                                                                                                                                                                                                           |

## Server → client

| `type`            | Payload                                                                                                                                              | Description                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                       |
| ----------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `hello`           | `{ protocol: number, current_persona: string \| null, available_personas: PersonaSummary[] }`                                                        | **First message on every connection.** Client checks `protocol` against its own `EXPECTED_PROTOCOL` constant and closes with code `1002` if they differ. Current version: `2`. **T1.13 additive fields** (old clients ignore unknown keys, so no version bump): `current_persona` is the name of the persona this connection is talking to (null only in the brief boot window before the first persona loads — clients today won't observe this). `available_personas` is `[{ name: string, display_name: string, language: string }, ...]` — every persona under `./personas/` the runtime can swap to. Re-enumerated per connection so a persona added while the server is running shows up without a restart. |
...
| `server_event`    | `{ event: string, data?: object }`                                                                                                                   | **Generic skill-emitted event channel** — the symmetric counterpart to inbound `client_event`. Skills push via `ctx.emit_server_event(key, data)`. Production clients (PWA, ESP32) tolerate unknown message types via their existing log-and-ignore default, so emit is safe to call regardless of which client is connected. **Namespacing**: same as `client_event` — `huxley.*` reserved (framework-emitted; none today), skills use `<skill-name>.*`. The PWA dev client surfaces incoming `server_event` in the existing dev-event log under `kind=server_event:<key>`.                                                                                                                                      |
| `sessions_list`   | `{ sessions: SessionMeta[] }`                                                                                                                        | **T1.12 — reply to `list_sessions`.** Each `SessionMeta`: `{ id: number, started_at: string (ISO), ended_at: string \| null, last_turn_at: string \| null, turn_count: number, preview: string \| null, summary: string \| null }`. Ordered most-recent first. The PWA renders relative time client-side from the raw ISO strings — the server doesn't pre-format. Sessions are scoped to the active persona (one DB per persona).                                                                                                                                                                                                                                                                                |
| `session_detail`  | `{ id: number, turns: Turn[] }`                                                                                                                      | **T1.12 — reply to `get_session`.** Each `Turn`: `{ idx: number, role: "user" \| "assistant", text: string }` in idx order. The PWA's `SessionDetailSheet` renders these as a read-only transcript. No streaming — the full transcript is returned in one message.                                                                                                                                                                                                                                                                                                                                                                                                                                                |
| `session_deleted` | `{ id: number }`                                                                                                                                     | **T1.12 — reply to `delete_session`.** Confirms the row + cascaded turns are gone so the PWA can drop it from local list state. No partial-failure shape — storage is local SQLite, succeeds or raises.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                           |
...
## Persona selection via query param (T1.13)

Persona is chosen at handshake time, not in-band. The client picks via the URL query string:

- `ws://host:port/ws` — connect to whatever persona the runtime currently hosts. The hello payload carries `current_persona` so the client can render the picker correctly.
- `ws://host:port/ws?persona=<name>` — explicitly request that persona. The server runs its swap algorithm (build new `Application` → start it → atomic ref-swap → background-teardown of old) BEFORE sending hello, so the hello payload reflects the requested persona.

Switching personas is therefore a **WebSocket reconnect**: the client closes its existing WS and opens a new one with the new `?persona=`. This mirrors the existing `?lang=<code>` flow exactly. There is no in-band `select_persona` message — the WS dropping for the ~50ms reconnect window is invisible to the user (the picker is gated on `!_claim_or_stream_active()`, so no audio is interrupted) and the cost of an in-band swap (audio-frame-during-swap concurrency hazards, atomic dispatcher rebinding, swap-lock) doesn't pay for itself. Rationale: [decision: hot persona swap via reconnect](./decisions.md#2026-05-01--hot-persona-swap-via-reconnect-not-in-band-t113).

**Default-persona resolution at boot** (when no `?persona=` is supplied AND no client has yet specified one):

1. `HUXLEY_PERSONA` env var — used verbatim.
2. Single persona under `./personas/` — autodiscovered.
3. Multiple personas + no env — alphabetic-first wins, with a loud `persona.default_picked_alphabetically` log line listing what was available. The PWA picker lets the user change immediately.
4. No personas — runtime refuses to start with an explanatory error.

Refusing to start at step 3 was rejected in design: it would force the env var on every multi-persona install, exactly the deployment artifact T1.13 retires.

**Failure mode**: if the requested `?persona=<name>` doesn't exist or fails to start, the runtime logs (`runtime.persona_load_failed` / `runtime.persona_start_failed`) and **keeps the previous active persona**. The connection still establishes; hello carries the actual `current_persona`. The PWA detects the mismatch (`current_persona !== requested`) and surfaces a retry — letting the WS handshake crash on a bad persona name would create a reconnect loop on the PWA side that's painful to recover from.

**`available_personas` in hello** is enumerated fresh per connection (filesystem listing is cheap). Adding a persona to `./personas/` while the server is running shows up in the next client connection without a server restart. Removing a persona that's currently `current_app` is undefined behavior — don't.
