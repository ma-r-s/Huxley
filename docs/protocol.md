...
| `state`          | `{ value: "IDLE" \| "CONNECTING" \| "CONVERSING" \| "PLAYING" }` | Server-side state machine's current state. Sent on every transition and once on client connect for initial sync.                                                                                         |
