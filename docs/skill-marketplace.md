...
- **v2 — caregiver expansion** (Phases A + B + C + D all shipped 2026-05-02). Layers a PWA Skills panel + install machinery + browseable registry on top of v1's primitives. Purely additive: new server endpoints, new PWA tabs. **No v1 rewrites.** Phase A (read-only state + cards UX), Phase B (writes via hot reload — toggle / config / secrets), Phase C (Marketplace tab consuming `huxley-registry`), and Phase D (`uv add` auto-install with `os.execv` self-restart) are all live.
...
Phases A, B, C, D all shipped 2026-05-02. The "What v2 adds" table below is now historical; each row's Status column carries the shipped state.
...
| Self-restart machinery  | `install_skill` server endpoint + `uv add` subprocess + `os.execv` re-exec. Required for venv changes; the in-process reload Phase B uses can't pick up new entry points. Mid-call gated via the `is_busy` accessor (covers both InputClaim and AudioStream). | **Shipped** (Phase D)     | None                                       |
| Curated registry browse | PWA Marketplace tab consumes the static `huxley-registry/index.json` feed (shipped at `ma-r-s/huxley-registry`). Cards with name + description + author + tier + installed-status badge.                                                                      | **Shipped** (Phase C)     | None — registry already exists             |
...
Phase D foot-guns (`os.execv` FD inheritance, JSON-Schema-doesn't-fit complex configs, registry maintainer load, dep conflicts, telegram-style first-time-auth, in-flight `inject_turn` on uninstall) were engaged in the Phase D planning critic + addressed in the implementation. The Phase D ADR in `docs/decisions.md` 2026-05-02 documents the foot-guns that landed mitigations and the ones deferred to post-D follow-ups.
