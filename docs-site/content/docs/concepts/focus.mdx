...
(Ducking is scaffolded but not fully implemented today. NONMIXABLE works.)
...
The original design (before the April 2026 pivot) tried to solve this with per-skill arbitration — an `Urgency` enum, a `YieldPolicy` enum, lots of small decisions everywhere. It was hard to reason about because the policies didn't compose.
